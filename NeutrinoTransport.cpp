...
int main() {
    ...
    string Model = "005"; // --- Determines Local Thermodynamic Conditions
    int N_g = 40;    // --- Number of Energy Groups (Must be 40)
    double t_end = 1.0; // [ s ] Final time
    ...
    double tolC = 1.0e-05;  // Particle Conservation Tolerance
    ...
    if (AppPert == true){
        ...
        if (Scheme == "ExplicitAsymptotic") {
            ...
            int Branch = FE_PE;
            BuildCollisionMatrix_NES(R_In, R_Out, Nold, dV, N_g, tolPE, cMat, kVec);
            ...
            for(int i = 0; i < N_g; i++) {
                for(int j = 0; j < N_g; j++) {
                    Nnew[i] = Nold[i] + dt * cMat[i][j] * Nold[i];
        }
    }
    //ForwardEulerupdate
    ...
    nTrueIterations = nTrueIterations + 1 ;
        ...
        for(int i = 0; i < N_g; i++) {
            for(int j = 0; j < N_g; j++) {
                Nnew[i] = Nold[i] + ( cMat[i][j] * Nold[i] ) * ( 1.0 - exp_kdt[i] ) / k0[i]; //qss1 update
            }
        }
            }
